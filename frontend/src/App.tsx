...
export function App() {
  ...
  const handleRefresh = () => {
    ...
    toast.success('Telemetry Synchronized', {
      description: `Refreshed at ${new Date().toLocaleTimeString()} • ${reports.length} reports, ${workers.length} workers.`,
    });
  };
    ...
    <BackendHealthGate>
      ...
      <div className="min-h-screen bg-[#fffaf0] text-[#0a0a0a] flex flex-col font-sans selection:bg-[#a4d4c5] selection:text-[#0a0a0a]">
        ...
        <header className="bg-[#fffaf0]/95 backdrop-blur-md border-b border-[#e5e5e5] sticky top-0 z-40 shadow-xs">
          <div className="max-w-7xl mx-auto py-2.5 px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
            
            {/* Logo & Clean Brand Identifier */}
            <div className="flex items-center gap-3 shrink-0">
              <div className="w-9 h-9 rounded-xl bg-[#0a0a0a] text-white flex items-center justify-center shadow-sm shrink-0">
                <span className="font-display font-black text-base tracking-tight text-[#a4d4c5]">
                  CL
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden md:inline-flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#faf5e8] text-[#0a0a0a] border border-[#e5e5e5]">
            ...
            <nav className="flex items-center bg-[#faf5e8] border border-[#e5e5e5] rounded-full p-1 shadow-xs shrink-0 overflow-x-auto">
              <button
                onClick={() => setActiveTab('dashboard')}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs transition-all whitespace-nowrap cursor-pointer ${
                  activeTab === 'dashboard'
            ...
            {/* Quick Actions (Uncluttered & Purposeful) */}
            <div className="flex items-center gap-2.5 shrink-0">
              {/* Staff Management */}
              <button
                onClick={() => setIsWorkersModalOpen(true)}
                className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl bg-[#faf5e8] text-[#0a0a0a] hover:bg-[#f5f0e0] border border-[#e5e5e5] transition-all active:scale-[0.98] shadow-xs whitespace-nowrap cursor-pointer"
                title="Manage Field Staff"
              >
                <Users className="w-3.5 h-3.5 text-[#1a3a3a] shrink-0" />
                <span>Staff <strong className="font-mono text-[#ff4d8b]">({workers.length})</strong></span>
              </button>

              {/* Polling / Last Sync Timestamp */}
              <div className="hidden lg:flex flex-col items-end text-[10px] font-mono text-[#6a6a6a] leading-tight pr-1">
                <span className="font-semibold text-[#0a0a0a]">LIVE SYNC</span>
              ...
              <button
                ...
                className="p-2 rounded-xl text-[#0a0a0a] bg-[#faf5e8] hover:bg-[#f5f0e0] border border-[#e5e5e5] transition-all active:scale-[0.95] shrink-0 cursor-pointer"
